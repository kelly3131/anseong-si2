            ...
            <li class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
                ...
                <div class="bo_cnt">
                    ...
                    <a href="<?php echo $list[$i]['href'] ?>" class="bo_subject">
						...
						<?php
						// pdf 파일이 있는지 체크
						$chk_wr_file_pdf = sql_fetch(" select count(bf_source) as cnt from `g5_board_file` where bo_table = '{$bo_table}' and wr_id = '{$list[$i]['wr_id']}' and substr(bf_source, -4) = '.pdf' ");
						?>
						<?php if ($chk_wr_file_pdf['cnt']) { ?>
						<span class="icon_view_pdf">PDF</span>
						<?php } ?>
